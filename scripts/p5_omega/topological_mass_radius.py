...
from ._vendored.pit import PiT_single

RESULTS = Path(__file__).resolve().parents[2] / 'results' / 'p5-omega-01' / 'regen'
Nc, Nf = 3, 2
...
def main():
    ...
    # ---------------- CSV / profile outputs ----------------
    with (RESULTS / 'topo_scan.csv').open('w', newline='') as fcsv:
    ...
    with (RESULTS / 'topo_ER_curve.csv').open('w', newline='') as fcsv:
    ...
    # profile data
    with (RESULTS / 'topo_profile_F2.csv').open('w', newline='') as fcsv:
    ...
    print("\nCSV: results/p5-omega-01/regen/topo_scan.csv, topo_ER_curve.csv, topo_profile_F2.csv")

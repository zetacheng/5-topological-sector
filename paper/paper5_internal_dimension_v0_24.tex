...
\large Working note v0.24 --- NOT a manuscript.\\
...
(existence and dynamization gates passed; the continuum-radius
gate has since RUN and FAILED---the full nonlocal radius scan
collapses to a cutoff-scale minimum, ``a lattice lump,'' with no
continuum window at any coupling; see the gate list).
...
gates.
v0.23 records a SECOND measurement-forced retraction, of this
note's Fierz entry: the report that the minimal chiral pair
generates identically zero vector and axial couplings was a
$\gamma_5$-without-the-$i$ convention slip.  The chirally
invariant pair Fierzes to $|c_V| = |c_A| = 1/2$; the cross-channel
ratios ARE Fierz-inherited, and the single-coupling economy
postulate erected on the false zero is withdrawn.v0.24 records the resolution of the last open item in the $\omega$
re-scoping: the continuum-radius gate \texttt{P5-OMEGA-01} has run
and FAILED (migrated from the legacy record and reviewer-verified
in this repository), excluding the $\omega$-stabilized continuum
soliton within the one-loop EFT and marking \texttt{P5-CL-008}
\texttt{FAILED}; the residual possibility is a cutoff-scale
lattice object, outside this construction's reach.}

\author{Zeta Hoi-Ho Cheng \\
\textit{Independent Researcher, Ontario, Canada}}

\date{July 2026 (v0.24)}
...
whose continuum-radius gate has since run and FAILED: within the
one-loop EFT the object collapses to a cutoff-scale lattice lump.
Continuum soliton existence is therefore \emph{excluded at this
order}, not open; no confirmation is claimed, and the residual
possibility is a genuine cutoff-scale (lattice) object, which this
EFT cannot address.
\end{abstract}
...
gate remains the outstanding requirement}, but its shape is
changed by the v0.23 Fierz retraction (see the gate list below):
the reported vanishing of the minimal pair's $V/A$ Fierz content
was a $\gamma_5$-without-the-$i$ slip, so the cross-channel ratios
\emph{are} Fierz-inherited after all and need not---and should
not---be supplied by a single-coupling economy postulate.  The
outstanding work is accordingly to recompute the ratios that the
correct chirally invariant decomposition delivers, and to re-ask
whether the resulting tower, combined with the vector-state
caveat, is rich enough for the $N \le 5$
...
solved here;
\emph{(Forward note, v0.23: verdict (ii) should be read narrowly.
What is dead is the hope of regenerating an \emph{attractive}
vector coupling that would bind a composite vector.  The repulsive
coupling this gauntlet found is not a dead end but the
$\omega$---its Hubbard--Stratonovich auxiliary dynamizes into a
healthy massive Proca-type vector, which the companion
vector-sector paper argues is a candidate, untuned, for the very
positive-$M_{\mathrm{Pl}}^2$ contribution the ``$G_V$ orphan problem''
names.  That reading is registered there as \texttt{PROPOSED},
not established; the sign, however, was never the obstacle it was
taken for here.)}
...
\emph{Provenance status (updated v0.23).}  These results have
since been migrated, with scope separated, into the companion
vector-sector paper's repository
(\url{https://github.com/zetacheng/3-vector-sector}), where they
are registered as gates \texttt{P3-FIERZ-01},
\texttt{P3-VEC-O1/O2/O3} and \texttt{P3-OMEGA-01}.  An independent
reviewer has reproduced $Z_\omega$, $M_\omega^2$, $g_{\omega B}$
and the pole residue in a clean environment, with
mutation-tested regression anchors and the archived artifacts
verified to stated tolerances; at the time of writing the
corresponding claims are recorded there as \texttt{SUPPORTED},
pending the principal investigator's acceptance of that verdict.
The ownership split adopted is that the companion paper owns the
$\omega$'s existence, health and repulsive kernel, while
\emph{this} note owns the question of whether it stabilizes a
topological object---gate \texttt{P5-OMEGA-01}, whose kill
criterion is the continuum-window placement below.
Two caveats that the companion paper records must be carried here
rather than assumed away: the four health gates hold for
\emph{any} $g_0 > 0$ and are coupling-independent, but the
below-threshold narrow pole requires
$g_0 > 1/(4m_f^2 Z_\omega) \simeq 10.6$, and the value used in
the artifacts is representative, not derived from the model's
$G/N$; at that representative coupling the $\omega$ mass sits at
$\simeq 1.4\,m_f$ rather than deep in a $m_f \ll m_\omega \ll
\Lambda$ hierarchy.  The distinction between computed and
independently certified is one this note takes seriously, since
both retractions above were caused by uncaught artifacts in
numbers that had looked stable.  In the \emph{local heavy-vector limit} the truncated landscape
...
matter).  \emph{That gate has since run and failed
(\texttt{P5-OMEGA-01}, \texttt{FAILED}; migrated and reviewer-verified
in this repository).}  With the exact negative $\kappa_U$, the
Skyrme term diverges as $-|B|/R$ while the $\omega$ repulsion
saturates---bounded in $R$ because the composite kernel is cut at
$\Lambda$, and bounded in the coupling because
$D_{00} = g_0/(1 + g_0\Pi_V) \to 1/\Pi_V$, so the RPA denominator
screens $g_0$: no coupling opens a continuum window, and the
minimum runs to the cutoff scale ($M_\omega R_\star = 0.87 < 1$,
$q_{\rm core}/\Lambda \approx 0.63$).  The collapse is structural,
not a scan artifact.  Soliton existence is therefore \emph{excluded
within this EFT}, not merely unclaimed; what remains open is only
the distinct cutoff-scale (lattice) question, which lies outside
the reach of this construction.
...
re-scoped the stabilization clause to the endogenous-$\omega$
landscape ($R_\star \gg a$); that re-scoped clause has since
FAILED its continuum gate (\texttt{P5-OMEGA-01}: cutoff-scale
collapse), so quartic Derrick stabilization is absent \emph{and}
its $\omega$-based replacement does not stabilize a continuum
object at this order.  The WZW/level half is unaffected.
...
stabilization clause---re-scoped to the $\omega$-stabilized
landscape with $R_\star \gg a$ as its quantitative
criterion---has since FAILED that criterion
(\texttt{P5-OMEGA-01}): the $\omega$ response saturates and the
minimum collapses to the cutoff scale, so no continuum soliton
survives at this order.  The second half---the level---is now anchored by a
...
\begin{enumerate}
\item \textbf{Fierz arithmetic: RETRACTED and re-COMPLETED
(v0.23).}  The v0.19 entry reported that the machine-verified
decomposition of the minimal chiral pair yields identically zero
vector and axial couplings, and inferred from this that the
cross-channel ratios $G_V/G_S$, $G_A/G_S$ are \emph{not}
Fierz-inherited and must instead be supplied by a single-coupling
economy postulate ($G_V = G_A = G_\chi$).  \emph{That entry is
withdrawn in its entirety.}  The zero was an artifact of Fierzing
$(\bar\psi\psi)^2 + (\bar\psi\gamma_5\psi)^2$, which is not the
chirally invariant operator: the hermitian pseudoscalar bilinear
is $\bar\psi i\gamma_5\psi$, so the chiral pair is
$(\bar\psi\psi)^2 + (\bar\psi i\gamma_5\psi)^2
 = (\bar\psi\psi)^2 - (\bar\psi\gamma_5\psi)^2$, and the
$(i)^2 = -1$ converts the scalar/pseudoscalar cancellation into an
addition.  With the standard Fierz table the two squares
contribute $\mp 1/4$ each to the vector channel, so the
non-invariant combination gives $0$ while the chirally invariant
one gives $|c_V| = |c_A| = 1/2$; the same machinery returns
exactly $0$ for $\gamma_5$ and $+1/2$ for $i\gamma_5$, on one
honest sign.  The correct statement is therefore the opposite of
the v0.19 one: \textbf{the ratios are Fierz-inherited}, the
economy postulate erected on the false zero is unnecessary as a
postulate and is withdrawn as a justification, and the
flavor-singlet value of the induced vector coupling is
$G_\omega = -G/N$ (companion vector-sector paper, gate
\texttt{P3-FIERZ-01}; re-derived there from $16$-dimensional
Dirac completeness with full reconstruction residual
$9\times10^{-16}$, and independently confirmed against the
standard Fierz table).  What survives of the v0.19 entry is only
its sub-question (b): the Aoki-risk operating-point analysis must
still be performed in the overlap discretization, where the $S/P$
sector is exactly degenerate and healthy.
\emph{Consequences not yet propagated}: any statement in this
programme that rests on the vanishing of the minimal pair's $V/A$
Fierz content, or on the economy postulate as its replacement,
requires re-examination; those statements are owned by the
companion papers and are flagged, not silently amended, here.
